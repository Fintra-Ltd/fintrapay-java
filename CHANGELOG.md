...
- Batch payouts accept `fee_deduction` (`BatchPayoutRequest.setFeeDeduction(...)`), applied to every recipient: `"from_amount"` (default) — each recipient gets their amount minus the per-recipient fee — or `"from_balance"` — each recipient gets exactly their amount and the fees are debited from your balance on top.

## 0.2.1 — 2026-05-02

### Documentation

- Webhook event bodies now include `external_id` — the value passed at invoice creation, or `null` when not supplied. Use it to correlate webhooks back to your own orders without parsing FintraPay's internal `invoice_id`. Verifier itself unchanged (signs opaque bytes); see `Webhook.java` example for usage.
